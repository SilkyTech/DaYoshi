<script lang="ts">
  ...
  function salvageAll(pet: number) {
    let savePet = $ownedPets[pet];
    let id = $ownedPets[pet][0];
    let newOwned: [number, number][] = (
    $ownedPets.map((a, i) => [a[0], i]) as [number, number][]
    ).filter((a) => a[0] === id);
    if (confirm(`Are you sure you want to sell all other pets of the same type? They will be salvaged for a total of ${(newOwned.length-1)*pets[$ownedPets[pet][0]].salvage}`)) {
...
<Modal bind:this={modal}>
  ...
  Level: {$curPet === -1 ? "None" : getLevels(boughtPets[$curPet]).level}<br />
  EXP to next Level: {$curPet === -1 ? "None" : getLevels(boughtPets[$curPet]).xp}<br />
  Description: {$curPet === -1 ? "None" : getLevels(boughtPets[$curPet]).desc}<br />
  Rarity: {$curPet === -1 ? "None" : boughtPets[$curPet]?.pet?.rarity}<br />
  <button on:click={() => salvage($curPet)}>Salvage</button>
  <button on:click={() => salvageAll($curPet)}
    >Salvage All Others of Same Type</button
  >
  <hr />
  <div class="pets">
    {#each boughtPets.sort((a, b) => b.xp-a.xp) as pet}
      <button
        ...
        disabled={pet.i === $curPet || ($ownedPets[pet.i][0] === 11 && !$usedDev)}
      >

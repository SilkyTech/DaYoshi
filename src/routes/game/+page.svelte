<script lang="ts">
	...
	function checkHealth() {
		if ($hp < 1) {
			...
			if ($curPet !== -1) {
				createLog(
					`Killed ${yoshiObj.name} +${yoshiObj.kill} Pet XP | ${levels.next - levels.xp}/${
						levels.next
					}`
				);
	...
	function tick() {
		...
		if (hand?.style !== undefined) hand.style.top = mouse.y + Math.sin(frame * 0.01) * 5 + 'px';
	...
	setInterval(tick, 0);
	$: yoshiObj =
		constants.yoshis[(curYoshi + constants.yoshis.length * 9999) % constants.yoshis.length];
	let cl2: any;
	let cps = 0;
	{
		let count = 0;
	...
	onMount(() => {
		...
		const interval: NodeJS.Timer = setInterval(() => {
			if (instance !== inst) return clearInterval(interval);
			...
			{
				if (
					constants.pets[$ownedPets[$curPet]?.[0]]?.perks(
						getLevelsNoLocal($ownedPets[$curPet]).level
					) !== undefined
				) {
					let base = 0;
...
>
	...
	<div class="main-info">
		...
		<span class="info-label"
			>HP: <div class="bar-full">
				<div class="bar-bar" style={`width: ${($hp / yoshiObj.hp) * 100}%;`} />
				<span>{$hp.toFixed(2)} / {yoshiObj.hp}</span>
			</div></span
		><br />
	...
	<div class="bottom-icons">
		<Tooltip label="Go back to home page">
			<CurvedButton onclick={() => location.href = "/"}>
		...
		<Tooltip label="Open Skin Menu">
			<CurvedButton onclick={() => shopComponent.open()}
				><svg
		...
		<Tooltip label="Open Pet Menu">
			<CurvedButton onclick={() => petmenuComponent.open()}
				><svg
					xmlns="http://www.w3.org/2000/svg"
					width="16"
					height="16"
		...
		<Tooltip label="Open Inventory">
			<CurvedButton onclick={() => inventoryComponent.open()}
				><svg
					xmlns="http://www.w3.org/2000/svg"
					width="16"
					height="16"
		...
		<Tooltip label="Open Settings">
			<CurvedButton onclick={() => settingsMenu.open()}
				><svg
					xmlns="http://www.w3.org/2000/svg"
					width="16"
					height="16"
	...
	{#if $potato.petAnim}
		<img
			src={pets?.[$ownedPets?.[$curPet]?.[0]]?.source ?? ''}
			alt=""
			class="pet-hand"
			bind:this={petfloat}
		/>
